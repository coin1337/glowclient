...
extends ToggleMod {
     ...
     */
    public Entity findTargetEntity(Vec3d selfPos, Vec3d selfLookVec) {
        ...
        synchronized (list) {
            for (Entity entity : Collections.synchronizedList(Lists.newArrayList((Iterable)world.loadedEntityList))) {
                Vec3d pos;
                if (entity == null || !this.isValidTarget(entity, pos = EntityUtils.getOBBCenter(entity), selfPos) || !priority.getMode().equals("Distance")) continue;
                double distance = pos.subtract(selfPos).normalize().subtract(selfLookVecNormal).length();
