                ...
                if (!(stack != null && (stack.getItem() instanceof ItemFood || stack.getItem() instanceof ItemAppleGold) && Globals.MC.gameSettings.keyBindUseItem.isKeyDown() || stack2 != null && (stack2.getItem() instanceof ItemFood || stack2.getItem() instanceof ItemAppleGold) && Globals.MC.gameSettings.keyBindUseItem.isKeyDown())) {
                    ...
                    for (Entity entity2 : entities) {
                        ...
                        for (BlockPos blockPos : blocks) {
                            ...
                            if (b >= 169.0 || !((d = (double)CrystalAura.calculateDamage((double)blockPos.getX() + 0.5, blockPos.getY() + 1, (double)blockPos.getZ() + 0.5, entity2)) > damage) || (self = (double)CrystalAura.calculateDamage((double)blockPos.getX() + 0.5, blockPos.getY() + 1, (double)blockPos.getZ() + 0.5, (Entity)Globals.MC.player)) > d && !(d < (double)((EntityLivingBase)entity2).getHealth()) || self - 0.5 > (double)Globals.MC.player.getHealth()) continue;
                            damage = d;
                            q = blockPos;
