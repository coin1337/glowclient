        ...
        if (arraylist.getBoolean()) {
            ...
            if (order.getMode().equals("Length")) {
                if (customfont.getBoolean()) {
                    ModuleManager.getMods().stream().filter(Module::isEnabled).filter(mod -> mod.isDrawn()).map(mod -> mod.getNameAndTag()).sorted(Comparator.comparing(SurfaceHelper::getTextWidthFont).reversed()).forEach(module -> {
                        if (colorMode.getMode().equals("Rainbow")) {
                            Color color = RainbowUtils.getRainbowColor(rainbowOffset.intValue() * 100000000, 1.0f);
                            this.r = color.getRed();
                            this.g = color.getGreen();
                    ...
                    ModuleManager.getMods().stream().filter(Module::isEnabled).filter(mod -> mod.isDrawn()).map(mod -> mod.getNameAndTag()).sorted(Comparator.comparing(SurfaceHelper::getTextWidth).reversed()).forEach(module -> {
                        if (colorMode.getMode().equals("Rainbow")) {
                            Color color = RainbowUtils.getRainbowColor(rainbowOffset.intValue() * 100000000, 1.0f);
                            this.r = color.getRed();
                            this.g = color.getGreen();
